...
find_identity(Response) :- 
	bagof(A, actor(A), Actors),
	bagof(L, link(L), Links),
	maplist(wikipedia_links(Links), Actors, ActorLinks),
	pairs_keys_values(Pairs, Actors, ActorLinks),
	query_oracle(Pairs, Response,[]), !,do_command([oscar, say, Response], _).

query_oracle([X-_|Xs], X, _) :-
	Xs = [].
query_oracle(Pairs, Response, Memory) :-
	agent_current_energy(oscar, Energy),
	(Energy < 30 -> solve_task(find(c(A)),_),agent_topup_energy(oscar,c(A)), Out = Pairs, Found = c(A);
	 otherwise -> 
	 	solve_task(find(o(A)),_),agent_ask_oracle(oscar, o(A), link, L),include(check_pair(L), Pairs, Out),Found=o(A)
	),
	query_oracle(Out, Response, Memory).

%find_ask(A, Memory).

check_pair(L, Key-Value) :-
	memberchk(L,Value).

wikipedia_links(PossibleLinks, Actor, Output) :-
	bagof(L, (wp(Actor, W),wt_link(W,L)), WikiLinks),
	intersection(PossibleLinks, WikiLinks, Output).
